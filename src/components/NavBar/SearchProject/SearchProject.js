...
const SearchProject = ({offset}) => {
    ...
    const scrolledStyle = offset ? "scrolled": ""
    return (
        <form className={`search-form ${scrolledStyle} `} onSubmit={onSubmit}>
            <input 
                className={`navbar-transition ${scrolledStyle} nav-search` }
                type="text" 
            ...
            <button type="submit" className="submit-search-button">
                <svg className={`navbar-transition ${scrolledStyle} svg-search-icon`}>
                    <path
